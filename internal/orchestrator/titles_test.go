...
func TestTitles(t *testing.T) {
	...
	cases := map[string]struct{ got, want string }{
		...
		"lock wait, holder unknown": {lockWaitTitle(0), "locked by another pull request, re-plan once it's released"},
		"not permitted":             {notPermittedTitle("runner.serviceAccount"), "runner.serviceAccount is not permitted"},
		"refused, one":              {refusedTitle("web", "runner.serviceAccount", 0), "not permitted: web sets runner.serviceAccount"},
		"refused, several":          {refusedTitle("web", "runner.serviceAccount", 2), "not permitted: web sets runner.serviceAccount, and 2 more"},
		"lock not acquired":         {lockNotAcquiredTitle(), "lock could not be acquired"},
		"record not saved":          {recordNotSavedTitle(), "operation could not be recorded"},
		"job not built":             {jobNotBuiltTitle(), "Runner Job could not be built"},
		"not started":               {notStartedTitle(), "operation could not be started"},
	}
